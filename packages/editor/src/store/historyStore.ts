...
import { applyPatches, produceWithPatches, enablePatches, type Patch } from 'immer'
...
export const useHistoryStore = create<HistoryStore>((set, get) => ({
  ...
  push(patches, inverses, label) {
    ...
    // 超出上限时裁剪
    if (newEntries.length > MAX_HISTORY) newEntries.shift()
  ...
  undo() {
    ...
    const newPage = applyPatches(currentPage, inverses) as PageSchema
  ...
  redo() {
    ...
    if (nextCursor >= entries.length) return
    const { patches } = entries[nextCursor]
    const currentPage = usePageStore.getState().page
    const newPage = applyPatches(currentPage, patches) as PageSchema
    usePageStore.getState().loadPage(newPage)
    set({
      cursor: nextCursor,
      canUndo: true,
...
export function withHistory<T extends unknown[]>(
  label: string,
  action: (...args: T) => void,
) {
  return (...args: T) => {
