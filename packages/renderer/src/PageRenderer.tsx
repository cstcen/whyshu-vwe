...
function renderChildren(node: ComponentNode, bp: Breakpoint) {
  return node.children?.map((child) => <RenderNode key={child.id} node={child} bp={bp} />)
}
...
function RenderNode({ node, bp }: RenderNodeProps): JSX.Element {
  ...
  switch (node.type) {
    case 'Root': {
      return <div style={style}>{renderChildren(node, bp)}</div>
    }
    ...
    case 'Container': {
      ...
      return (
        <div style={{ ...style, flexDirection: direction }}>
          {renderChildren(node, bp)}
        </div>
      )
    }

    case 'Section':
      return <section style={style}>{renderChildren(node, bp)}</section>
    ...
    case 'Header':
      return <header style={style}>{renderChildren(node, bp)}</header>

    case 'Nav': {
      const orientation = node.props.orientation ?? 'horizontal'
      return (
        <nav style={{ ...style, flexDirection: orientation === 'vertical' ? 'column' : 'row' }}>
          {renderChildren(node, bp)}
        </nav>
      )
    }
    ...
    case 'CarouselItem':
      return <div style={{ ...style, width: '100%', height: '100%' }}>{renderChildren(node, bp)}</div>
    ...
    case 'Footer':
      return <footer style={style}>{renderChildren(node, bp)}</footer>
    ...
    case 'Grid': {
      ...
      return (
        <div style={{ ...style, gridTemplateColumns: `repeat(${columns}, 1fr)`, gap }}>
          {renderChildren(node, bp)}
        </div>
      )
    }
    ...
    case 'Heading': {
      const Tag = (node.props.level as keyof JSX.IntrinsicElements) ?? 'h2'
      ...
      return (
        <span
          style={{
            ...
            fontSize: '12px',
            fontWeight: 500,
            background: 'color-mix(in srgb, var(--primary, #7c6af5) 14%, white)',
            color: 'var(--primary, #7c6af5)',
            ...style,
          }}
        >
          {String(node.props.text ?? '标签')}
        </span>
      )
    ...
    case 'Video': {
      ...
      return src ? (
        <video src={src} poster={String(node.props.poster ?? '')} style={style} controls />
      ) : (
        <div style={{ ...style, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280', fontSize: '14px' }}>
          视频占位（填写视频地址后显示）
        </div>
      )
    }
    ...
    case 'InputField': {
      ...
      return (
        <div style={style}>
          {fieldLabel && <label style={{ fontSize: '13px', fontWeight: 500, color: 'var(--text, #374151)', display: 'block', marginBottom: '4px' }}>{fieldLabel}</label>}
          <input type={String(node.props.inputType ?? 'text')} placeholder={String(node.props.placeholder ?? '')} style={INPUT_STYLE} readOnly />
        </div>
      )
    }
    ...
    case 'TextareaField': {
      ...
      return (
        <div style={style}>
          {fieldLabel && <label style={{ fontSize: '13px', fontWeight: 500, color: 'var(--text, #374151)', display: 'block', marginBottom: '4px' }}>{fieldLabel}</label>}
          <textarea placeholder={String(node.props.placeholder ?? '')} rows={Number(node.props.rows ?? 4)} style={{ ...INPUT_STYLE, resize: 'vertical' }} readOnly />
        </div>
      )
    }
    ...
    case 'Checkbox':
      return (
        <label style={style}>
          <input type="checkbox" defaultChecked={!!node.props.checked} style={{ width: '16px', height: '16px', accentColor: 'var(--primary, #7c6af5)' }} readOnly />
    ...
    case 'Alert': {
      ...
      const colors = {
        info: { bg: 'color-mix(in srgb, var(--primary, #7c6af5) 14%, white)', border: 'var(--primary, #7c6af5)', text: 'var(--primary, #7c6af5)', icon: 'i' },
        success: { bg: '#dcfce7', border: '#22c55e', text: '#14532d', icon: 'v' },
        warning: { bg: '#fef3c7', border: '#f59e0b', text: '#78350f', icon: '!' },
        error: { bg: '#fee2e2', border: '#ef4444', text: '#7f1d1d', icon: 'x' },
      }
      const color = colors[variant as keyof typeof colors] ?? colors.info
      return (
        <div style={{ ...style, backgroundColor: color.bg, borderColor: color.border, color: color.text }}>
          <span style={{ fontSize: '20px', fontWeight: 700, marginRight: '8px' }}>{color.icon}</span>
          <div style={{ flex: 1 }}>
    ...
    case 'Avatar': {
      ...
      const avatarStyle = { ...style, width: dimension, height: dimension }
      return src ? (
        <img src={src} alt={alt} style={avatarStyle} />
      ) : (
        <div style={{ ...avatarStyle, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#9ca3af', color: '#fff', fontWeight: 700 }}>
          {fallback}
        </div>
      )
    }
    ...
    case 'Tabs': {
      ...
      return (
        <div style={style}>
          ...
          <div style={{ padding: '16px 0' }}>{renderChildren(node, bp)}</div>
        </div>
      )
    }
...
export function PageRenderer({ schema, breakpoint }: PageRendererProps) {
  ...
  return (
    <div style={{ ...(cssVarStyle ?? {}), background: 'var(--bg, #ffffff)', color: 'var(--text, #1a1a1a)' }}>
